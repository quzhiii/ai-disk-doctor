...
当前还提供 `aidisk models adapters` 能力报告：默认仅检查本地 refs/manifest 元数据。显式指定 `--probe-official-cli` 后，只调用 `hf` / `ollama` 的 `--version` 与 `--help`，并受超时、输出上限约束；不执行 prune/delete，不修改官方索引。
