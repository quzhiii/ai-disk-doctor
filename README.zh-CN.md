...
- **依赖清晰** — Unix 治理入口依赖 bash、jq、curl 和 cargo；具体 notifier adapter 仍留到后续阶段。
...
| **跨平台定时治理** | 通过 Windows Task Scheduler、cron、launchd 或 systemd timer 调度本地治理，不引入后台 daemon 或具体 notifier adapter |
