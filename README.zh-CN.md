...
- **依赖清晰** — Unix 治理入口依赖 bash、jq、curl 和 cargo；当前版本已经包含 local-file、generic webhook 和 Feishu notifier adapter，更广的平台扩展仍留到后续阶段。
...
| **跨平台定时治理** | 通过 Windows Task Scheduler、cron、launchd 或 systemd timer 调度本地治理，不引入后台 daemon，并通过 local-file、generic webhook 或 Feishu 投递稳定治理事件 |
...
Unix-like 治理入口依赖 `bash`、`jq`、用于 webhook 或 Feishu 投递的 `curl`，以及用于本地运行的 `cargo`。cron、launchd 和 systemd timer adapter 只会注册调用 `run-governance.sh` 的平台原生调度任务；它们不会引入后台 daemon，也不会执行清理。Phase 13 已经包含具体的 Feishu notifier adapter，而更广的 adapter 扩展仍留到后续阶段。
