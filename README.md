...
- `models adapters` only inspects local metadata by default. With explicit `--probe-official-cli`, it invokes only bounded `hf` or `ollama` version/help commands. With explicit `--run-official-dry-run`, it runs only allowlisted non-mutating commands: `hf cache prune --dry-run --cache-dir <root>` after help confirms `--dry-run`, or `ollama ls` as a read-only list. Hugging Face dry-run output can be normalized into a report-only cleanup plan; Ollama list output never becomes a cleanup candidate. It never performs cleanup or mutates indexes, and all actions remain `report-only`.
