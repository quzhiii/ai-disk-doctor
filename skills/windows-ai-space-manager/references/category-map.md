...
- `ai-model`: `doctor --agents` 或 `scan --category ai-model`，覆盖 LM Studio / llama.cpp 等 GPU 推理运行器缓存与下载模型，以及 GGUF / SafeTensors / ONNX / MLX 等模型权重文件
- `ai-runtime`: `scan --category ai-runtime`，覆盖 CUDA / cuDNN / NVIDIA 计算工具包等 AI 运行时环境，通常体积大、多版本共存、需通过官方工具卸载
