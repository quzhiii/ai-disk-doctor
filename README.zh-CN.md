...
当前 `master` 在已发布的 v1.4.0 跨平台治理基础上，继续增加了 Notifier Adapter Foundation：

- **Notifier Adapter Foundation** — `scripts/governance/send-governance-event.sh` 可将 `governance-event.json` 分发到 `local-file`、generic webhook 或 Feishu。
- **Feishu adapter** — `scripts/governance/notifiers/feishu.sh` 通过环境变量 `FEISHU_WEBHOOK_URL` 发送 `headline` 和 `summary_markdown`，不使用命令行传 secret。
- **Secrets 边界** — `feishu-failure.json` 只保留失败上下文，不记录 resolved webhook URL。

当前主线的 notifier adapter 说明见 [`docs/notifier-adapters.md`](./docs/notifier-adapters.md)。
