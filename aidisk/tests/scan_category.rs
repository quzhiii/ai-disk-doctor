...
fn playwright_fixture_matches_glob_pattern_shape() {
    ...
    fs::write(fixture.join("chromium.zip"), vec![1_u8; 1024])
...
fn repository_fixtures_cover_core_storage_shapes() {
    ...
    assert!(root
        .join("AppData/Local/Packages/DistroA/LocalState/ext4.vhdx")
        .exists());
    assert!(root
        .join("AppData/Local/Docker/build-cache/cache.db")
        .exists());
    assert!(root
        .join("AppData/Local/Docker/wsl/disk/docker_data.vhdx")
        .exists());
    assert!(root
        .join("projects/demo-app/.playwright-browsers/chromium.zip")
        .exists());
    assert!(root
        .join(".cache/huggingface/hub/models--demo/model.bin")
        .exists());
