...
Date: 2026-08-12  
Reality audit baseline: `master` at `c29c750a3c0f88c2ea60d8a119f02cfd9aea9997`
...
- Current repo fact: verified in this repository during the M0 audit.
...
- Current repo fact: the CLI commands are wired in `aidisk/src/main.rs`: `scan`, `plan`, `clean`, `restore`, `diff`, `anomaly`, `doctor`, `rules`, `models`, and `visualize`.
- Current repo fact: the Rust Core includes scanner, planner, cleaner/quarantine/restore, doctor, model inventory, diff, anomaly, history, reporter, rules, rules repo, policy, and HTML visualization modules.
...
| Scanner, planner, cleaner, restore, doctor, diff, anomaly, history, model inventory, reporter, visualize are implemented | `aidisk/src/*.rs` modules and CLI wiring | true |
...
|---|---|---|
| One execution truth for CLI and Desktop | CLI/Core owns execution today | Core services are still mostly invoked through `main.rs`; future Desktop needs a shared application/domain interface |
