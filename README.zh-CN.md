...
v1.6.0 增加 AI 感知诊断和可视化仪表盘：

- **可视化仪表盘** — `aidisk visualize --html` 生成交互式瑞士风 HTML 仪表盘，支持中英双语、类别筛选、可安全回收清单。
- **AI 足迹** — `aidisk doctor --ai-footprint` 聚合 10 个 AI 类别发现。
...
v1.5.0 增加跨平台治理能力，包含 Feishu 通知投递和治理可靠性增强：

- **Feishu notifier** — `scripts/governance/notifiers/feishu.sh` 通过环境变量 `FEISHU_WEBHOOK_URL` 投递 `governance-event.json`。
- **治理可靠性** — 事件去重防止重复投递；可配置重试（最多 3 次，60s 间隔）包装通知投递。
- **跨平台 CI** — GitHub Actions 现在同时测试 Windows、Ubuntu、macOS。
- **用户手册** — `docs/governance-manual.md` 覆盖四个调度平台、notifier adapter、去重、重试和故障排查。

完整说明：[CHANGELOG.md](./CHANGELOG.md) · [Release Notes v1.5.0](./docs/release-notes/v1.5.0.md)。

### v1.4.0

v1.4.0 增加跨平台定时治理能力，同时保持不自动清理的治理边界：

- **跨平台定时治理** — cron、launchd、systemd timer adapter 现在与 Windows Task Scheduler 一样提供注册、查看、卸载和立即测试运行流程。
- **Unix 治理入口** — `run-governance.sh` 提供 Unix-like 平台的 scan -> anomaly -> `governance-event.json` -> generic webhook 工作流，并与 Windows `run-governance.ps1` 保持契约对齐。
- **Scheduler-first 边界** — 调度适配器仍然是脚本层、平台原生实现，不引入后台 daemon。
- **依赖清晰** — Unix 治理入口依赖 bash、jq、curl 和 cargo；已发布的 v1.4.0 仍不包含具体平台 notifier adapter。

完整说明：[CHANGELOG.md](./CHANGELOG.md) · [Release Notes v1.4.0](./docs/release-notes/v1.4.0.md)。

### v1.3.0

Phase 9 增加本地定时治理能力，同时保持不自动清理的安全边界：

- **增长异常检测** — `aidisk anomaly` 基于绝对 + 相对双阈值对比扫描快照。
- **本地治理工作流** — `run-governance.ps1` 执行 scan → anomaly → report artifact 生成。
- **稳定治理事件** — `governance-event.json` 承载 `anomaly_found`、`pending_history`、`no_anomaly`，并包含适合消息消费的摘要字段。
- **通用 webhook 投递** — 治理事件可投递到 generic webhook，失败时保留 `webhook-failure.json`。
- **Windows 调度辅助脚本** — 支持注册、查看、卸载并立即测试运行本地治理任务。

完整说明：[CHANGELOG.md](./CHANGELOG.md) · [Release Notes v1.3.0](./docs/release-notes/v1.3.0.md)。
...
| **跨平台定时治理** | 通过 Windows Task Scheduler、cron、launchd 或 systemd timer 调度本地治理，不引入后台 daemon；v1.4.0 提供 local-file 与 generic webhook，当前主线文档额外覆盖 Feishu adapter foundation |
